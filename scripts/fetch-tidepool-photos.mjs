#!/usr/bin/env node
/**
 * Fetch CC-licensed reference photos for the tide-pool species from iNaturalist
 * and stage them for review, with full attribution. Nothing is uploaded or
 * committed automatically.
 *
 * Why: the "flip a rock" reveal card shows a pixel sprite by default and a real
 * photo when one is available (src/lib/hobbies/tidepool-photos.json). This finds
 * properly-licensed candidates so you can drop real photos into those tiles.
 *
 * Usage:
 *   node scripts/fetch-tidepool-photos.mjs [options]
 *   npm run fetch:tidepool-photos -- [options]
 *
 * Options:
 ...
 *   --wire MODE     Also write src/lib/hobbies/tidepool-photos.json:
 ...
 *
 * Output:
 *   staging/tide-pooling/photos/<id>.<ext>   downloaded photo (gitignored)
 *   staging/tide-pooling/photos.json         review manifest with attribution
 *
 * Next steps after reviewing the manifest:
 *   - hotlink iNaturalist:  re-run with `--wire inat`
 *   - host them yourself:   upload staging/tide-pooling/photos/* to
 *                           hobby-media/tide-pooling/species/, then `--wire blob`
 *
...
const UA = 'photography-website tidepool photo fetcher (+https://trumanbrown.com)';
const DATASET = 'src/lib/hobbies/tidepool-species.ts';
const OUT_DIR = join('staging', 'tide-pooling', 'photos');
const MANIFEST = join('staging', 'tide-pooling', 'photos.json');
const WIRING = 'src/lib/hobbies/tidepool-photos.json';
const BLOB_BASE = 'hobby-media/tide-pooling/species';
...
function parseSpecies(src) {
  const re = /id:\s*'([^']+)'\s*,\s*common:\s*'([^']*)'\s*,\s*scientific:\s*'([^']+)'/g;
  const out = [];
  let m;
  while ((m = re.exec(src))) out.push({ id: m[1], common: m[2], scientific: m[3] });
  return out;
}
...
/** Find the best CC-licensed photo for a scientific name. */
async function findPhoto(scientific, licenses) {
  ...
  let data;
  try {
    data = await apiJson(`${API}/observations?${q}`);
  } catch {
    return null;
  }
...
async function main() {
  ...
  for (const sp of species) {
    ...
    const hit = await findPhoto(sp.scientific, args.licenses);
  ...
  if (args.wire === 'inat' || args.wire === 'blob') {
    const blobHost = (await readFile('site.config.ts', 'utf8').catch(() => '')).match(
      /blobHost:\s*'([^']+)'/,
    )?.[1];
    const wiring = {
      $comment:
        'id -> { photo, credit }. Used by the tide-pool reveal card; falls back to the pixel sprite if a photo fails to load. Generated/updated by scripts/fetch-tidepool-photos.mjs.',
      species: {},
    };
    ...
    for (const [id, m] of Object.entries(manifest)) {
      ...
      else if (args.wire === 'blob' && blobHost) photo = `https://${blobHost}/${m.blobUrl}`;
      if (photo) {
    ...
    if (args.wire === 'blob' && !blobHost) {
      console.log('  (could not read blobHost from site.config.ts — nothing wired)');
    }
  }
  ...
  if (!args.wire) {
    ...
    console.log(`  • host them yourself:   upload ${OUT_DIR}/* to ${BLOB_BASE}/, then --wire blob`);
  }
}

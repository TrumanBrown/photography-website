...
4. **Choose the content path.** Trusted events install `libraw-bin`, log into Azure via OIDC, restore the prebuild cache, and run `scripts/prebuild.mjs` against Blob. The completed prebuild cache is saved immediately for the next run, then its local copy is deleted because the images have already been materialized under `src/content/`. Dependabot and fork PRs cannot receive those secrets, so they run `scripts/generate-fixtures.mjs` instead. Both paths produce schema-valid session content for the same production build command.

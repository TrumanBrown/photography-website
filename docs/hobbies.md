...
**One scene recipe, two renderers.** Both canvases consume the same `HikeState`, so the developed image cannot silently invent a different route. The responsive editor uses the same terrain-sampled forests, meadows, lakes, and snowfields at reduced branch/grass/bank detail so direct dragging stays fast; the export canvas is created only on request at a fixed `2400 × 1600`. Postcard mode adds contour bands, cloud shadows, hundreds of scree marks, gullies, ledges, biome-specific waterfalls/vines/snow streaks, trail stones, footprints, denser vegetation, and foreground depth. Lakes find the spill line inside their actual carved basin and use the sampled basin floor as their submerged boundary, behind irregular banks, stones, and reeds. Forests use individually grounded conifers with local slope, root shadow, altitude-aware size, varied age, and feathered edges; meadows are clipped grass-and-heather bands that follow and extend down the local face rather than horizontal decals. Snowfields sample the local ridgeline and descend as broken, shadowed tongues with exposed rock instead of flat overlays. Generation uses only the visitor's CPU/GPU and browser memory. It makes no API calls, uploads nothing, and incurs no per-generation Azure cost.
